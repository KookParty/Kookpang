    ...
    <body>
      ...
      <script>
        ...
        function sum() {
          ...
          const el = qs('#sum'); if (el) el.textContent = price(s);
        }

        document.addEventListener('DOMContentLoaded', () => {
          try { initHeader && initHeader('recipes'); } catch (e) { }
          ...
          window.addSelected = function () {
            ...
            checked.forEach(ch => {
              const row = ch.closest('.row');
              const id = row.dataset.id, title = row.dataset.title, price = Number(row.dataset.price || 0);
              const i = cart.findIndex(x => x.id === id);
              if (i > -1) cart[i].qty = (cart[i].qty || 1) + 1;
            ...
            try {
              const n = document.createElement('div'); n.textContent = `${"${checked.length}"}개 재료가 장바구니에 추가되었습니다.`;
