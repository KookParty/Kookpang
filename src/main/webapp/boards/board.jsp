...
<html lang="ko">
  <head>
    ...
    <style>
      ...
      #kp-board-pinned .card.pinned h3 {
        font-size: 18px;
        margin: 0;
      }
      #kp-board-pinned .card.pinned .small {
        font-size: 13px;
      }
      #kp-board-pinned .kp-pinned-header {
        display: flex;
      ...
      .board-controls .search-wrap {
        flex: 1;
        min-width: 200px;
      }
      .board-controls .search-wrap .input {
        width: 100%;
  ...
  <body>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", () => {
        if (typeof initHeader === "function") initHeader("board");
      });
    </script>
        ...
        <div class="board-controls">
          ...
          <div class="actions">
            ...
            <a
              class="btn dark"
              id="kp-board-write"
              href="${path}/front?key=board&methodName=writeForm"
    ...
    <script>
      if (!window.__KP_BOARD_LIST_INIT__) {
        window.__KP_BOARD_LIST_INIT__ = true;
        (function () {
          var page = 1,
            size = 10;
          ...
          document.addEventListener("click", function (e) {
            if (e.target && e.target.closest && e.target.closest("#kp-sort-menu")) return;
          ...
          function renderPinned(rows) {
            ...
            for (var i = 0; i < rows.length; i++) {
              ...
              html +=
                ...
                '<a class="kp-post-link" href="' +
                BASE +
                "/front?key=board&methodName=view&postId=" +
                r.postId +
          ...
          function renderAppend(rows) {
            ...
            for (var i = 0; i < rows.length; i++) {
              ...
              html +=
                ...
                escapeHtml(r.nickname || "익명") +
                "</p>" +
                "<p>" +
                escapeHtml(stripHtml(r.content || "").slice(0, 140)) +
                "</p>" +
          ...
          async function fetchList() {
            ...
            var url =
              BASE +
              "/ajax?key=board&methodName=listData&category=" +
              ...
              if (j.ok) {
                ...
                if (page === 1 && j.pinned && Array.isArray(j.pinned) && j.pinned.length) {
                  ...
                  var pinnedIds = new Set(
                    j.pinned.map(function (p) {
                      return p.postId;
                    })
                  );
                  if (Array.isArray(j.rows) && j.rows.length) {
                    j.rows = j.rows.filter(function (r) {
                      return !pinnedIds.has(r.postId);
                    });
                  }
                }
