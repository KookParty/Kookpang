  ...
  (function () {
    ...
    function updateAuthUI(user) {
      ...
      if (user) {
        ...
        IS_AUTH = true;
        if ($nick) {
          $nick.textContent = user.nickname || user.email || "사용자";
        ...
        if ($mypage)
          $mypage.href = BASE + "/users/login.jsp?redirect=" + encodeURIComponent(location.pathname + location.search);
      }
    }
      ...
      document.querySelectorAll("[data-need-auth]").forEach((el) => {
        el.addEventListener("click", (e) => {
          if (!IS_AUTH) {
            ...
            location.href =
              BASE + "/users/login.jsp?redirect=" + encodeURIComponent(location.pathname + location.search);
          }
        });
      });
    }
